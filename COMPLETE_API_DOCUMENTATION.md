...
12. [Backtesting APIs](#backtesting-apis)
13. [Trading Mode Toggle API](#trading-mode-toggle-api)
14. [Bot Status API](#bot-status-api)
...
The backtesting module enables historical strategy simulation using real market data. Paper trading mode must be enabled (`trading.paper-trading-enabled=true`). All endpoints require the `X-User-Id` header.
  ...
  "data": {
    ...
    "tradeEvents": [
      ...
      },
      {
        ...
        "eventType": "EXIT",
        "description": "Strategy completed - TARGET_HIT",
        "prices": {},
        "totalValue": 15900.0,
        "unrealizedPnL": 1150.0
      }
    ]
  }
}
...
**Error Cases:**
- `400`: Validation failures (missing strategyType, instrument, etc.).
- `409`: Paper trading mode disabled.
- `500`: Historical data unavailable or internal failure; response includes `errorMessage`.
...
**Behavior:**
- Defaults to parallel execution using a dedicated executor (limit configured via `backtesting.max-concurrent-batch-backtests`).
- Each entry mirrors the single-backtest request payload.
- `runSequentially=true` forces ordered backtests (useful for deterministic logs).
...
```json
{
  ...
  "data": {
    ...
    "endTime": "2025-11-23T10:08:00",
    "totalDurationMs": 480000,
    "totalBacktests": 2,
    "successfulBacktests": 2,
    "failedBacktests": 0,
    "results": [
      { "backtestId": "...", "status": "COMPLETED", "performanceMetrics": { "netProfitLoss": 1000.0, "returnPercentage": 6.78 } },
      { "backtestId": "...", "status": "COMPLETED", "performanceMetrics": { "netProfitLoss": 650.0, "returnPercentage": 4.10 } }
    ],
    "aggregateStatistics": {
      "totalNetPnL": 1650.0,
      "averageNetPnL": 825.0,
      "totalReturnPercentage": 10.88,
      "averageReturnPercentage": 5.44,
      "totalWins": 2,
      "totalLosses": 0,
      "winRate": 100.0,
      "bestReturn": 6.78,
      "worstReturn": 4.10,
      "averageHoldingDurationMs": 5400000.0,
...
Returns the recorded execution metadata and cached response for a prior backtest. Useful for polling UI workflows.
...
```json
{
  ...
  "data": {
    ...
    "endTime": "2025-11-23T10:35:00",
    "status": "COMPLETED",
    "response": { /* BacktestResponse payload */ }
  }
}
```

### 4. Backtest Health Check

**Endpoint:** `GET /api/backtest/health`

Returns `200` with message "Backtesting service is available" when the service is ready and paper trading mode is enabled.
...
backtesting:
  enabled: true
  default-replay-speed: 0
  default-include-detailed-logs: false
  max-concurrent-batch-backtests: 10
  batch-executor-pool-size: 10
```

Historical replay uses `historical.replay.sleep-millis-per-second` (set to `0` for fastest). All backtests require `trading.paper-trading-enabled=true`.
